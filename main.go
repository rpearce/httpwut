...
func main() {
	...
	if len(os.Args) == 1 || os.Args[1] == "" || os.Args[1] == "help" || needsHelp {
		usage()
	} else {
		var isVerbose bool
		var statusMessage, statusLong, statusUrl string

		statusCodeFlagSet := flag.NewFlagSet("statusCode", flag.ExitOnError)
		statusCodeFlagSet.BoolVar(&isVerbose, "verbose", false, helpVerbose)
		statusCodeFlagSet.BoolVar(&isVerbose, "v", false, helpVerbose)
		...
		default:
			statusMessage = "Error: Unknown HTTP status code"
			errorCount++
		}
