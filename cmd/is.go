...
var isCmd = &cobra.Command{
	Use:   "is",
	Short: "Lookup HTTP status codes",
	Long:  "",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if status, exists := statuses[statusCode]; exists {
			...
			if hasCats, _ := cmd.Flags().GetBool("cats"); hasCats {
			...
			if hasDogs, _ := cmd.Flags().GetBool("dogs"); hasDogs {
			...
			return fmt.Errorf("HTTP status code not found: \"%s\"", statusCode)
		}
...
func init() {
	...
	isCmd.Flags().BoolP("verbose", "v", false, "Print status code description and URL")
	isCmd.Flags().BoolP("cats", "c", false, "Open HTTP Status Cats webpage for status")
	isCmd.Flags().BoolP("dogs", "d", false, "Open HTTP Status Dogs webpage for status")
}
